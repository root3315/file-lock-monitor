...
def read_file_with_timeout(path: str, timeout: int = 5) -> Optional[str]:
    """Read a file with timeout protection to avoid hanging on locked files."""
    def handler(signum, frame):
        raise TimeoutError(f"Read timeout on {path}")
...
def get_process_name(pid: int) -> str:
    ...
    content = read_file_with_timeout(f"/proc/{pid}/comm")
    if content:
...
def get_process_user(pid: int) -> str:
    ...
    content = read_file_with_timeout(f"/proc/{pid}/status")
    if content:
...
def scan_proc_locks(timeout: int = 5) -> List[LockInfo]:
    ...
    for pid_str in proc_dirs:
        ...
        content = read_file_with_timeout(locks_path, timeout=timeout)
        if not content:
            continue
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Timeout in seconds for file operations (default: 5)"
    )
    ...
    if args.proc_scan:
        proc_locks = scan_proc_locks(timeout=args.timeout)
